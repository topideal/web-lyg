$(function () {
    getDataList();
    uploadBtn("uploadFileBtn", "fileName");



    $('#saveDataBtn').click(function () {
        var parms = $("#addFm").serializeFmObject();
        if (parms.isVideo == null) {
            parms.isVideo = 1;
        }
        $.ajax({
...
function getDataList() {
    $('#data_list').DataTable({
        ...
        "sAjaxSource": "/info/getInfoFileListByPaginator.do",
        "fnServerData": function (url, aoData, fnCallback) {
            $.ajax({
                url: url,//这个就是请求地址对应sAjaxSource
                data: {jsonParam: JSON.stringify(aoData),infoId:$("#infoId").val()},
                type: 'POST',
                dataType: 'json',
                async: false,
        ...
        "aoColumnDefs": [
            {
                ...
                "mRender": function (data, type, full) {
                    return "<input type='checkbox' value='" + data.infoFileId + "' />";
                }
            },
            {
                "aTargets": [1],
                "mRender": function (data, type, full) {
                    if (data.isVideo == 0) {
                        return viewVideo(data.fileName);
                    } else {
                        return viewImg(data.fileName);
                    }
                }
            },
            {
